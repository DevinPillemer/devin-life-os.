...
// Intensity color: maps minutes to a bg color
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-[10px] font-bold text-white tabular-nums">{completed}/{target}</span>
      </div>
    </div>
  );
}
  ...
  if (loading) {
    return (
      <div className="rounded-xl bg-surface border border-slate-700/50 p-5">
        ...
        <div className="h-20 skeleton rounded mb-4" />
    ...
    <div className="rounded-xl bg-surface border border-slate-700/50 p-5">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="text-center">
          <div className="text-lg font-bold text-white tabular-nums">{health?.sessions || 0}</div>
          <div className="text-[10px] text-slate-400 uppercase">Sessions</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-bold text-white tabular-nums">{health?.totalKm || 0}km</div>
          <div className="text-[10px] text-slate-400 uppercase">Distance</div>
        </div>
        <div className="text-center">
          <div className="text-lg font-bold text-white tabular-nums">{health?.totalMinutes || 0}m</div>
      ...
      <div className="flex items-center gap-2 mb-4 text-xs">
        <span className={`flex items-center gap-0.5 font-medium ${vsLastWeek.kmDelta >= 0 ? "text-emerald-400" : "text-red-400"}`}>
          {vsLastWeek.kmDelta >= 0 ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
          {vsLastWeek.kmDelta >= 0 ? "+" : ""}{vsLastWeek.kmDelta}km
        </span>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {weekChart.map((day: any, i: number) => (
            <div key={i} className="text-center">
              <div className="text-[9px] text-slate-500 mb-1">{day.day}</div>
              <div
                className={`aspect-square rounded-md flex items-center justify-center text-[9px] text-slate-300 font-medium ${intensityColor(day.minutes)}`}
                title={`${day.km}km / ${day.minutes}min`}
              >
          ...
          <div key={i} className="flex items-center justify-between text-xs py-1.5">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <span className="text-slate-500 ml-1.5 capitalize">{a.type}</span>
              </div>
            </div>
            <div className="text-slate-500 tabular-nums">
              {a.km > 0 ? `${a.km}km · ` : ""}{a.minutes}min
            </div>
          </div>
        ))}
      </div>
